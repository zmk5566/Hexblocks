...
HexBlocks is an open-source software-hardware framework for authoring modular creative sensor-actuator systems. Hardware modules describe their capabilities, a hub maintains stable UID-keyed references, and a browser authoring UI uses a live schema to validate rules generated by a large language model. The reference implementation uses hex-shaped magnetic blocks on a 500 kbps CAN bus with a battery-powered hub. The hub speaks the same `$`-prefixed line protocol over USB-CDC serial or Bluetooth Low Energy. In hub-resident mode, an Event-Condition-Action (ECA) runtime evaluates rules locally and persists the program in NVS. In sensor-stream mode, parsed sensor and actuator events flow through a Python bridge runtime to the browser and to OSC-based creative-coding environments.

## Links

- Paper: to be added after upload
...
@misc{hexblocks2026,
  ...
  archivePrefix = {arXiv},
  primaryClass  = {cs.HC},
  howpublished  = {arXiv link to be added after upload}
}
```

/*
 * WearBlocks Motor Hub v1 — one-face hub + built-in DRV8410 motors
 * Target: ESP32-C3FH4
 *
 * Architecture:
...
static const uint8_t FACE_COUNT = 1;
static const uint8_t FACE_PIN = 5;
...
WearBlocksECA      eca;
WearBlocksDescriptor builtinMotorDescriptor;
uint32_t builtinMotorUid = 0;
...
static void applyMotorOutputs(uint8_t index) {
...
static void setMotor(uint8_t motor, uint8_t mode, uint8_t speed,
    ...
    applyMotorOutputs(index);
    Serial.printf("[MOTOR] local M%d mode=%d speed=%d duration=%u\n",
                  motor, state.mode, state.speed, durationMs);
}

static bool handleLocalActuator(uint32_t targetUid, uint8_t cmd,
                                const uint8_t* payload, uint8_t payloadLen) {
    if (targetUid != builtinMotorUid || cmd != ACT_MOTOR_SET) return false;
...
static void tickMotors() {
    ...
    for (uint8_t i = 0; i < 2; i++) {
        if (motors[i].untilMs != 0 &&
            ...
            applyMotorOutputs(i);
            Serial.printf("[MOTOR] local M%d duration complete\n", i + 1);
        }
    }
}

static void setupBuiltinMotorDescriptor() {
    // moduleId is a type label (16-byte field); UID carries instance identity.
    strlcpy(builtinMotorDescriptor.moduleId, "motor_builtin",
            sizeof(builtinMotorDescriptor.moduleId));
    strlcpy(builtinMotorDescriptor.name, "Built-in Dual Motor",
            sizeof(builtinMotorDescriptor.name));
    strlcpy(builtinMotorDescriptor.category, "motor_output",
            sizeof(builtinMotorDescriptor.category));
    strlcpy(builtinMotorDescriptor.color, "#D97757",
            sizeof(builtinMotorDescriptor.color));
    strlcpy(builtinMotorDescriptor.version, "1.0",
            sizeof(builtinMotorDescriptor.version));

    builtinMotorDescriptor.numCapabilities = 1;
    WBCapability& motor = builtinMotorDescriptor.capabilities[0];
    ...
    builtinMotorDescriptor.numAffordances = 2;
    strlcpy(builtinMotorDescriptor.affordances[0], "independent_motor_control",
            sizeof(builtinMotorDescriptor.affordances[0]));
    strlcpy(builtinMotorDescriptor.affordances[1], "built_in",
            sizeof(builtinMotorDescriptor.affordances[1]));
}
...
FaceTrack faceTracks[FACE_COUNT] = {};
...
// Explicit prototypes keep Arduino's .ino preprocessor from placing these
// declarations before AttachKind is defined.
void enqueueAttach(AttachKind k, uint8_t hubFace,
                   uint8_t parentSlot, uint8_t parentFace);
void pruneAttach(AttachKind k, uint8_t hubFace,
                 uint8_t parentSlot, uint8_t parentFace);
...
void onModuleHello(const HelloMessage& msg) {
    ...
    if (mem) {
        ...
        if (parentSlotMem == WB_PARENT_HUB) {
            if (mem->parentFace >= 1 && mem->parentFace <= FACE_COUNT) {
...
static void emitBuiltinMotor() {
    char uid[9];
    formatUidHex(builtinMotorUid, uid, sizeof(uid));
    out.printf("$H,%s,%s,HUB,0\n", uid, builtinMotorDescriptor.moduleId);
    out.printf("$I,%s,%s,%s,0000\n", uid,
               builtinMotorDescriptor.moduleId,
               builtinMotorDescriptor.version);
    out.printf("$D,%s,%s\n", uid, builtinMotorDescriptor.toJSON().c_str());
}
...
bool handleEcaCommand(const char* cmd, size_t len) {
    ...
    if (strncmp(cmd, "$A ", 3) == 0) {
        ...
        if (count >= 1) {
            ...
            if (localTarget) {
                if (handleLocalActuator(targetUid, c, params,
                                        (uint8_t)paramCount)) {
                    out.printf("$OK A local cmd=%d\n", c);
                } else {
                    out.println("$ERR A unsupported_local_cmd");
                }
            } else {
...
void setup() {
    ...
    Serial.println();
    Serial.println("========================================");
    Serial.println("  WearBlocks Motor Hub v1 (UID-keyed)");
    Serial.println("========================================");
    ...
    // Keep the bridge asleep until every PWM output is known-low.
    ...
    if (!pwmOk) {
        Serial.println("[FAIL] motor PWM init failed!");
        while (1) delay(1000);
    }
    digitalWrite(MOTOR_SLEEP_PIN, HIGH);
    delayMicroseconds(200);

    builtinMotorUid = (uint32_t)ESP.getEfuseMac();
    setupBuiltinMotorDescriptor();
    ...
    pinMode(FACE_PIN, INPUT_PULLUP);
    Serial.printf(" F1=GPIO%d", FACE_PIN);
    Serial.println();

    delay(20);
    bool occ = (digitalRead(FACE_PIN) == LOW);
    faceTracks[0].committedOccupied = occ;
    if (occ) {
        enqueueAttach(ATTACH_HUB_FACE, 1, 0, 0);
        Serial.println("  F1 initial: OCCUPIED (queued)");
    }
    ...
    eca.begin(protocol);
    eca.setLocalActuatorHandler(handleLocalActuator);
